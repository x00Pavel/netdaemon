...
[NetDaemonApp]
[Focus]
public sealed class PerfTestAppSecond
{
    private readonly ILogger<HelloApp> _logger;
    private Stopwatch _stopwatch = new();
    public PerfTestAppSecond(IHaContext ha, ILogger<HelloApp> logger)
    {
        _logger = logger;
        ...
        ha.StateAllChanges().Where(n => n.Entity.EntityId == "binary_sensor.vardagsrum_pir")
            .Subscribe(s =>
            {
                if (s.New?.State == "on")
                {
                    ...
                    {
                        _stopwatch.Stop();
                        _logger.LogInformation("Performance measured APP2: {MsgPerSecond} subscriptions/s ({NrOfMessagesReceived})", nrOfMessagesReceived/_stopwatch.Elapsed.TotalSeconds, nrOfMessagesReceived);
                    }
                }
                else
                {
